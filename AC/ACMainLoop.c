	...
	CW_REPEAT_FOREVER
	  {
		...
		CWDebugLog("Entering ManageWTP loop...");
		int readBytes;
		...
		while ((gWTPs[i].isRequestClose == CW_FALSE) &&
			 ...
			 /*TODO: Check system */
			CWDebugLog("Waiting for WTP...");
			CWWaitThreadCondition(&gWTPs[i].interfaceWait, 
					      &gWTPs[i].interfaceMutex);
			CWDebugLog("Waiting complete...");
		}
		...
		if (CWGetCountElementFromSafeList(gWTPs[i].packetReceiveList) > 0) {
			...
			switch(gWTPs[i].currentState) 
			{
				...
				case CW_ENTER_RUN:
				{
					...
					{
						CWDebugLog("ACEnterRun Returned FALSE");
						if(CWErrorGetLastErrorCode() == CW_ERROR_INVALID_FORMAT) 
						{
							/* Log and ignore other messages */
							CWErrorHandleLast();
							CWLog("AC --> Received something different from a valid Run Message");
						} 
						else 
						{
							/* critical error, close session */
						...
						CWDebugLog("ACEnterRun Completed Successfully.");
					}
					break;
				}
				default:
		  ...
		  if (gWTPs[i].interfaceCommand != NO_CMD) {
			...
			case WTP_UPDATE_CMD:
				...
				/*
								 ...
								 *
								 */
								case CW_UPDATE_CONTROL_CMD: {
									int seqNum = CWGetSeqNum();
									if (CWAssembleMessageNew(&(gWTPs[i].messages),
													&(gWTPs[i].messagesCount), gWTPs[i].pathMTU, seqNum,
													CW_MSG_TYPE_VALUE_CONFIGURE_UPDATE_REQUEST,
													gWTPs[i].msgElems, gWTPs[i].msgElemCount,
													gWTPs[i].msgElemLen, CW_PACKET_CRYPT)) {
										if (CWACSendAcknowledgedPacket(i,
