...
/*
 * Added by Navneet
 * Initiate sending update to WTP
 * AC will send Configuration update message elements
 * directly to WTP without parsing them internally.
 */

int CWUPDATE_SetValues(int wtpIndex, int socketIndex, char* update_msgElems, int msgElemCount, int msgElemLen) {

	CWDebugLog("entered CWUPDATE_SetValues\n\n\n\n");

	CWThreadMutexLock(&(gWTPs[wtpIndex].interfaceMutex));

	gWTPs[wtpIndex].interfaceCommand = CW_UPDATE_CONTROL_CMD;
	gWTPs[wtpIndex].applicationIndex = socketIndex;
	gWTPs[wtpIndex].msgElems = update_msgElems;
	gWTPs[wtpIndex].msgElemCount = msgElemCount;
	gWTPs[wtpIndex].msgElemLen = msgElemLen;

	CWSignalThreadCondition(&gWTPs[wtpIndex].interfaceWait);
	CWWaitThreadCondition(&gWTPs[wtpIndex].interfaceComplete,
...
CW_THREAD_RETURN_TYPE CWManageApplication(void* arg) {
	...
	char* update_msgElems;
	
	int payload_size;
	int i, nameLength, numActiveWTPs=0, wtpIndex, elemLen, msgElemCount;
	...
	CW_REPEAT_FOREVER { 
		...
		if ( ( n = Readn(sock, &cmd_msg, CMD_TYPE_SIZE) ) > 0 ) {
			...
			/*
						 * Added by Navneet:
						 * AC read the first bit for CONF_UPDATE_MSG
						 * and receives all message elements as it is.
						 * AC then compile a CAPWAP control message
						 * using control message headers and the
						 * message elements sent by extern APP (WUM)
						 */
						if (cmd_msg == CONF_UPDATE_MSG) {
							/************************
							 * Manage CONF_UPDATE_MSG
							 * ---------------------
							 * 1. Gather all binding msg elems
							 * 2. Get Index of WTP
							 * 3. Manage request
							 */

							//get WTP index
							if ((n = Readn(sock, &wtpIndex, sizeof(wtpIndex))) < 0) {
								CWLog("Error while reading wtpIndex from socket.");
								goto quit_manage;
							}
							wtpIndex = ntohl(wtpIndex);
							CWDebugLog("WTP Index received from WUM: %d\n", wtpIndex);
							...
							// get number of msgElemCount
							if ((n = Readn(sock, &msgElemCount, sizeof(msgElemCount))) < 0) {
								CWLog("Error while reading msgElemCount length from socket.");
								goto quit_manage;
							}
							msgElemCount = ntohl(msgElemCount);
							CWDebugLog("msgElemCount received from WUM: %d\n", msgElemCount);

							//get size of msgElems
							if ((n = Readn(sock, &elemLen, sizeof(elemLen))) < 0) {
								CWLog("Error while reading msgElem length from socket.");
								goto quit_manage;
							}
							elemLen = ntohl(elemLen);
							CWDebugLog("ElemLen received from WUM: %d\n", elemLen);

							// receive complete msgElems
							//memset(update_msgElems, 0, elemLen);
							update_msgElems = (char*) malloc(elemLen);
							if ((n = Readn(sock, update_msgElems, elemLen)) < 0) {
								CWLog("Error while reading msgElem from socket.");
								goto quit_manage;
							}

							CWDebugLog("value of n bytes received %d \n", n);

							//TODO-Navneet: extend for all WTPs case
							CWUPDATE_SetValues(wtpIndex, socketIndex, update_msgElems, msgElemCount, elemLen);

							CW_FREE_OBJECT(update_msgElems);
							//CW_FREE_OBJECT(msgElemCount);
							msgElemCount = 0;
