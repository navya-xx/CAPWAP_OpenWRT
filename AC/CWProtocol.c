...
// Assemble a CAPWAP Control Packet, with the given Message Elements, Sequence Number and Message Type. Create Transport and Control Headers.
// completeMsgPtr is an array of fragments (can be of size 1 if the packet doesn't need fragmentation
CWBool CWAssembleMessageNew(CWProtocolMessage **completeMsgPtr,
		int *fragmentsNumPtr, int PMTU, int seqNum, int msgTypeValue,
		char *msgElemsBinding, const int msgElemBindingNum,
		const int msgElemBindingLen, int is_crypted) {
	CWProtocolMessage transportHdr, controlHdr, msg;
	int i;
	...
	if (completeMsgPtr == NULL || fragmentsNumPtr == NULL || (msgElemsBinding
			== NULL && msgElemBindingNum > 0))
		return CWErrorRaise(CW_ERROR_WRONG_ARG, NULL);

	//Assemble Control Header
	controlVal.messageTypeValue = msgTypeValue;
	controlVal.msgElemsLen = msgElemBindingLen;
	controlVal.seqNum = seqNum;
	...
	// assemble the message putting all the data consecutively
	CW_CREATE_PROTOCOL_MESSAGE(msg, controlHdr.offset + msgElemBindingLen, return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););

	CWProtocolStoreMessage(&msg, &controlHdr);
	CWProtocolStoreRawBytes(&msg, msgElemsBinding, msgElemBindingLen);
	...
	if (PMTU > 0) {
		...
		*fragmentsNumPtr = msg.offset / PMTU;
		...
		cw_dont_fragment: *fragmentsNumPtr = 1;
	}
	...
	if (*fragmentsNumPtr == 1) {
		...
		transportVal.isFragment = transportVal.last
				= transportVal.fragmentOffset = transportVal.fragmentID = 0;
		transportVal.payloadType = is_crypted;
		//		transportVal.last = 1;
		...
		for (i = 0; i < *fragmentsNumPtr; i++) { // for each fragment to assemble
			...
			CWProtocolStoreMessage(&((*completeMsgPtr)[i]), &transportHdr);
			CWProtocolStoreRawBytes(&((*completeMsgPtr)[i]),
					&((msg.msg)[msg.offset]), fragSize);
			msg.offset += fragSize;
