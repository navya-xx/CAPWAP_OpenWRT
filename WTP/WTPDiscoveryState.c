...
CWBool CWParseDiscoveryResponseMessage(char *msg, int len, int *seqNumPtr,
	...
	CW_REPEAT_FOREVER {
		...
		for (i = 0; i < gCWACCount; i++) {
			...
			if (!(gCWACList[i].received)) {
				...
				if (!CWErr(
						CWAssembleDiscoveryRequest(&msgPtr,
								gCWACList[i].seqNum))) {
					exit(1);
				}

				CW_CREATE_OBJECT_ERR(gACInfoPtr, CWACInfoValues,
						return CW_QUIT;);
				...
				j = CWErr(
						CWNetworkSendUnsafeUnconnected(gWTPSocket,
								&(gACInfoPtr->preferredAddress), (*msgPtr).msg,
								(*msgPtr).offset));
				/* 
...
CWBool CWReceiveDiscoveryResponse() {
	...
	if (!CWErr(
			CWNetworkReceiveUnsafe(gWTPSocket, buf, CW_BUFFER_SIZE-1, 0, &addr, &readBytes))) {
		return CW_FALSE;
	}

	CW_CREATE_OBJECT_ERR(ACInfoPtr, CWACInfoValues,
			return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););

	/* check if it is a valid Discovery Response */
	if (!CWErr(
			CWParseDiscoveryResponseMessage(buf, readBytes, &seqNum,
					ACInfoPtr))) {
...
void CWWTPPickACInterface() {
	...
	cw_pick_IPv4: if (gACInfoPtr->IPv4Addresses == NULL
			|| gACInfoPtr->IPv4AddressesCount <= 0)
		return;
...
CWBool CWAssembleDiscoveryRequest(CWProtocolMessage **messagesPtr, int seqNum) {
	...
	return CWAssembleMessage(messagesPtr, &fragmentsNum, 0, seqNum,
	CW_MSG_TYPE_VALUE_DISCOVERY_REQUEST, msgElems, msgElemCount,
...
CWBool CWParseDiscoveryResponseMessage(char *msg, int len, int *seqNumPtr,
	...
	if (controlVal.messageTypeValue != CW_MSG_TYPE_VALUE_DISCOVERY_RESPONSE)
		return CWErrorRaise(CW_ERROR_INVALID_FORMAT,
				"Message is not Discovery Response as Expected");
	...
	*seqNumPtr = controlVal.seqNum;
	...
	CW_CREATE_ARRAY_ERR(ACInfoPtr->IPv4Addresses, ACInfoPtr->IPv4AddressesCount,
	...
	if (ACInfoPtr->IPv6AddressesCount > 0) {
		...
		CW_CREATE_ARRAY_ERR(ACInfoPtr->IPv6Addresses,
				ACInfoPtr->IPv6AddressesCount, CWProtocolIPv6NetworkInterface,
				return CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL););
	}
