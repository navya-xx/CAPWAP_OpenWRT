...
#define 	CW_DATA_MSG_STA_INFO_TYPE				4 /* 2015 update */
...
typedef enum {
	...
	CW_PROTOCOL_FAILURE_UNRECOGNIZED_MSG_ELEM = 21,
	// Failure: unrecognized message element
	 ...
	 *
	 */
	CW_BINDING_RESULT_SUCCESS = 22, // Binding Successfully implemented
	CW_BINDING_RESULT_FAILURE = 23  // Binding implementation failed
...
typedef struct {
	...
	enum {
		CW_WTP_MODEL_NUMBER = 0, CW_WTP_SERIAL_NUMBER = 1, CW_BOARD_ID = 2, CW_BOARD_REVISION = 3,

		CW_WTP_HARDWARE_VERSION = 0, CW_WTP_SOFTWARE_VERSION = 1, CW_BOOT_VERSION = 2
	} type;
	int length;
	int *valuePtr;
...
typedef enum {
	CW_LOCAL_BRIDGING = 1, CW_802_DOT_3_BRIDGING = 2, CW_NATIVE_BRIDGING = 4, CW_ALL_ENC = 7
} CWframeTunnelMode;

typedef enum {
	CW_LOCAL_MAC = 0, CW_SPLIT_MAC = 1, CW_BOTH = 2
} CWMACType;

typedef struct {
	enum {
		CW_MSG_ELEMENT_DISCOVERY_TYPE_BROADCAST = 0, CW_MSG_ELEMENT_DISCOVERY_TYPE_CONFIGURED = 1
	} type;
...
typedef enum {
	CW_X509_CERTIFICATE = 1, CW_PRESHARED = 0
} CWAuthSecurity;
...
typedef struct {
	int vendorIdentifier;
	enum {
		CW_AC_HARDWARE_VERSION = 4, CW_AC_SOFTWARE_VERSION = 5
	} type;
	int length;
	int *valuePtr;
...
typedef enum {
	AD_NORMAL = 1, AD_RADIO_FAILURE = 2, AD_SOFTWARE_FAILURE = 3, AD_RADAR_DETECTION = 4
} CWAdminCause;

typedef enum {
	OP_NORMAL = 0, OP_RADIO_FAILURE = 1, OP_SOFTWARE_FAILURE = 2, OP_ADMINISTRATIVELY_SET = 3
} CWOperationalCause;
...
CWBool CWProtocolParseFragment(char *buf, int readBytes, CWList *fragmentsListPtr,
		CWProtocolMessage *reassembledMsg, CWBool *dataFlag);
void CWProtocolDestroyFragment(void *f);

CWBool CWParseTransportHeader(CWProtocolMessage *msgPtr,
		CWProtocolTransportHeaderValues *valuesPtr, CWBool *dataFlag);
CWBool CWParseControlHeader(CWProtocolMessage *msgPtr, CWControlHeaderValues *valPtr);
CWBool CWParseFormatMsgElem(CWProtocolMessage *completeMsg, unsigned short int *type,
...
CWBool CWAssembleMessage(CWProtocolMessage **completeMsgPtr, int *fragmentsNumPtr, int PMTU,
		int seqNum, int msgTypeValue, CWProtocolMessage *msgElems, const int msgElemNum,
...
CWBool CWAssembleUnrecognizedMessageResponse(CWProtocolMessage **messagesPtr, int *fragmentsNumPtr,
		int PMTU, int seqNum, int msgType);
...
CWBool CWAssembleVendorMsgElemResultCodeWithPayload(CWProtocolMessage *msgPtr,
		CWProtocolResultCode code, CWProtocolVendorSpecificValues *payload);//49
...
CWBool CWParseWTPRadioOperationalState(CWProtocolMessage *msgPtr, int len,
		CWRadioOperationalInfoValues *valPtr); //30
